...
```bash
node {{SKILLS_DIR}}/hooks-framework/scripts/context-check.mjs
node {{SKILLS_DIR}}/hooks-framework/scripts/lint-check.mjs
# Tool output offloading (JSON input)
echo '{"tool_output":"Large output content...","tool_name":"test"}' | node {{SKILLS_DIR}}/hooks-framework/scripts/tool-offload.mjs
```
