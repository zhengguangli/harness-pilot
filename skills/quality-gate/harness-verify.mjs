...
const reactCount = countPattern(join(root, '{{AGENTS_DIR}}/orchestrator.md'), 'ReAct')
const hooksDir = join(root, '{{SKILLS_DIR}}/hooks-framework')
const toolOffload = existsSync(hooksDir)
  ? readdirSync(join(hooksDir, 'scripts')).filter(f => f.endsWith('.mjs')).length > 0 ? 1 : 0
  : 0
const browser = countPattern(join(root, '{{SKILLS_DIR}}/sandbox-exec/SKILL.md'), 'chromium')
const filesystem = countPattern(join(root, '{{SKILLS_DIR}}/context-setup/SKILL.md'), '文件系统是最基础')
...
for (const hook of hooks) {
  checkFile(`${hook}.mjs`, join(root, `{{SKILLS_DIR}}/hooks-framework/scripts/${hook}.mjs`))
}
...
for (const agent of agents) {
  checkFile(agent, join(root, `{{AGENTS_DIR}}/${agent}.md`))
}
...
for (const skill of skills) {
  checkFile(skill, join(root, `{{SKILLS_DIR}}/${skill}/SKILL.md`))
}
...
// 检测 hooks 配置文件
const configFiles = [
  { path: '.claude/settings.json', name: 'Claude Code' },
  { path: '.codex/hooks.json', name: 'Codex' },
  { path: '.opencode/plugins/harness-hooks.ts', name: 'OpenCode' }
]
...
for (const config of configFiles) {
  ...
  if (existsSync(configPath)) {
    try {
      ...
      const hasHooks = content.includes('hook') || content.includes('Hook')
      if (hasHooks) {
        console.log(`  ${GREEN}✓${RESET} ${config.path}: 含 hooks 配置`)
        pass++
        hooksFound = true
      } else {
        console.log(`  ${YELLOW}!${RESET} ${config.path}: 存在但无 hooks`)
        warn++
      }
    } catch {
      console.log(`  ${RED}✗${RESET} ${config.path}: 读取失败`)
      fail++
    }
  }
}
if (!hooksFound && configFiles.every(c => !existsSync(join(root, c.path)))) {
  console.log(`  ${YELLOW}!${RESET} 未找到 hooks 配置文件`)
  warn++
}
