...
node {{SKILLS_DIR}}/entropy-gc/scripts/drift-scan.mjs --quick

# Weekly full scan (all dimensions)
node {{SKILLS_DIR}}/entropy-gc/scripts/drift-scan.mjs --full

# Generate quality score
node {{SKILLS_DIR}}/entropy-gc/scripts/quality-score.mjs
```
...
jobs:
  scan:
    ...
    steps:
      ...
      - name: Drift scan
        run: |
          node {{SKILLS_DIR}}/entropy-gc/scripts/drift-scan.mjs ${{ steps.scan.outputs.mode }}
...
# crontab -e
# Daily quick scan
0 8 * * * cd /path/to/project && node {{SKILLS_DIR}}/entropy-gc/scripts/drift-scan.mjs --quick
# Weekly full scan
0 8 * * 1 cd /path/to/project && node {{SKILLS_DIR}}/entropy-gc/scripts/drift-scan.mjs --full && node {{SKILLS_DIR}}/entropy-gc/scripts/quality-score.mjs
```
