...
- **Files as handoff**: Agents pass intermediate artifacts through the `.workspace/` directory
...
- Intermediate artifacts in `.workspace/` (auditable)
...
- Final artifacts written to project-specified paths; intermediate artifacts kept in `.workspace/`
...
Agent output → Check if complete?
  ...
  └─ Incomplete / Early exit →
        ...
        Create new context
        Inject: original prompt + progress files from .workspace/
        Re-invoke agent
```
...
3. Offload completed intermediate steps to `.workspace/`
...
1. Write full output to `.workspace/tool_output/{timestamp}_{tool}.txt`
...
**Transport Layer:**
- **Filesystem (default)**: Share data via `.workspace/` directory — low latency, auditable, supports resume from checkpoint
...
|--------------|-----------------|----------------|
| `request` | Write to target agent's `.workspace/inbox/` | Target agent replies with `response` |
| `response` | Write to requesting agent's `.workspace/inbox/` | No ack needed |
| `notify` | Broadcast to `all`, write to `.workspace/broadcast/` | No ack needed |
| `error` | Write to orchestrator's `.workspace/inbox/` + circuit breaker counter | Orchestrator decides handling strategy |
