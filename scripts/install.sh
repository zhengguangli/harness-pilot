...
main() {
  ...
  echo ""
  echo "  支持的命令 — 对 AI 说这些召唤 skill:"
  echo ""
  echo "    🎯 初始化 / 搭建"
  echo "    \"初始化 harness\"        — harness-init  全量搭建 agent 团队"
  echo "    \"搭建 harness\"          — harness-init  同上"
  echo "    \"运行 harness\"          — orchestrator  编排 agent 团队执行"
  echo "    \"harness run\"           — orchestrator  同上"
  echo ""
  echo "    📚 知识库"
  echo "    \"设置知识库\"            — context-setup 生成 AGENTS.md / docs"
  echo "    \"生成 AGENTS.md\"        — context-setup 同上"
  echo "    \"创建文档结构\"          — context-setup 同上"
  echo ""
  echo "    🏗️ 架构"
  echo "    \"架构检查\"              — architecture-guard 执行边界验证"
  echo "    \"边界验证\"              — architecture-guard 同上"
  echo "    \"生成 lint 规则\"        — architecture-guard 同上"
  echo "    \"品味检查\"              — architecture-guard 同上"
  echo ""
  echo "    🧹 维护"
  echo "    \"垃圾收集\"              — entropy-gc    代码清理 / 技术债务"
  echo "    \"代码清理\"              — entropy-gc    同上"
  echo "    \"漂移检测\"              — entropy-gc    同上"
  echo "    \"技术债务\"              — entropy-gc    同上"
  echo ""
  echo "    ✅ 质量"
  echo "    \"质量审查\"              — quality-gate  代码审查门禁"
  echo "    \"代码审查\"              — quality-gate  同上"
  echo "    \"质量检查\"              — quality-gate  同上"
  echo ""
  echo "    🔍 可读性"
  echo "    \"智能体可读性\"          — agent-readability 优化 AI 可读性"
  echo "    \"agent readability\"     — agent-readability 同上"
  echo "    \"让智能体看懂\"          — agent-readability 同上"
  echo ""
  echo "    📊 可观测性"
  echo "    \"配置可观测性\"          — observability-setup 日志 / 监控 / 追踪"
  echo "    \"设置日志\"              — observability-setup 同上"
  echo "    \"监控配置\"              — observability-setup 同上"
  echo ""
  echo "    🔗 钩子 / 中间件"
  echo "    \"配置 hooks\"            — hooks-framework 确定性执行钩子"
  echo "    \"中间件\"                — hooks-framework 同上"
  echo "    \"执行钩子\"              — hooks-framework 同上"
  echo ""
  echo "    🧪 沙箱"
  echo "    \"沙箱\"                  — sandbox-exec   安全代码执行"
  echo "    \"安全执行\"              — sandbox-exec   同上"
  echo ""
  echo "    🔄 演进"
  echo "    \"改进 harness\"          — harness-evolve 反馈驱动演进"
  echo "    \"harness evolve\"        — harness-evolve 同上"
  echo "    \"反馈整合\"              — harness-evolve 同上"
  echo ""
}
